...
# --- End Defaults ---
# --- Preset configs for homework runs ---
# These presets hard-code input paths, checkpoints, output dirs, thresholds, etc.
# You only need to specify: --dataset and --model

PRESET_CONFIGS = {
    # 1) KITTI – PointPillars (car only)
    "pointpillars_hv_secfpn_8xb6-160e_kitti-3d-car": {
        "dataset": "kitti",
        ...
        "out_dir": "outputs/kitti_pointpillars",
        "modality": "lidar",
        "score_thr": 0.3,
        "device": "cuda:0",
        "headless": True,
    ...
    # 3) nuScenes sample – PointPillars
    "pointpillars_hv_fpn_sbn-all_8xb4-2x_nus-3d": {
        # Internally uses "any" dataset mode
        "dataset": "any",
        "input_path": "data/nuscenes_demo/lidar/n015-2018-07-24-11-22-45+0800__LIDAR_TOP__1532402927647951.pcd.bin",
        "frame_number": None,
        "checkpoint": "checkpoints/nuscenes_pointpillars/hv_pointpillars_fpn_sbn-all_4x8_2x_nus-3d_20210826_104936-fca299c1.pth",
        "out_dir": "outputs/nuscenes_pointpillars",
        "modality": "lidar",
        "score_thr": 0.2,
        "device": "cuda:0",
        "headless": True,
    ...
    # 4) nuScenes sample – CenterPoint
    "centerpoint_voxel01_second_secfpn_head-circlenms_8xb4-cyclic-20e_nus-3d": {
        # Also uses "any" dataset mode with the same lidar sample
        "dataset": "any",
        "input_path": "data/nuscenes_demo/lidar/n015-2018-07-24-11-22-45+0800__LIDAR_TOP__1532402927647951.pcd.bin",
        "frame_number": None,
        ...
        "out_dir": "outputs/nuscenes_centerpoint",
        "modality": "lidar",
        "score_thr": 0.2,
        "device": "cuda:0",
        "headless": True,
...
def apply_preset_from_model(args):
    """
    If args.model matches one of our PRESET_CONFIGS keys, override
    the other arguments so you only need to type --dataset and --model.
    """
    preset = PRESET_CONFIGS.get(args.model)
    if not preset:
        return args  # no preset, just use whatever was passed on CLI

    print(f"Applying preset settings for model '{args.model}'")

    # Fill args.* from preset
    for key, value in preset.items():
        if value is None:
            continue  # skip None fields like frame_number for nuscenes
...
if __name__ == "__main__":
    ...
    args = apply_preset_from_model(args)
